...
objective objectives_stack[NUM_OBJECTIVES] = {
    ...
    {"WE'LL TAKE OUR CHANCES IN THE SHUTTLE", DROP_COOLANT, false,
     "DOCKING BAY", 0, 0, 0},
    {"CREW MEETING", CREW_AT_LOCATION_WITH_MINIMUM_SCRAP, false, "GALLEY", 0, 0,
     1},
    {"WHAT'S THE DAMAGE?", CREW_AT_LOCATION_WITH_MINIMUM_SCRAP, false,
     "WORKSHOP", 0, 0, 0},
    {"DRIVE 'EM INTO THE AIRLOCK", BRING_ITEM_TO_LOCATION, false, "AIRLOCK", 0,
     INCINERATOR, 0},
    {"WHERE IS IT?", BRING_ITEM_TO_LOCATION, false, "MED BAY", 0, FLASHLIGHT,
     0},
    {"SHOULDN'T HAVE LANDED ON THIS BALL", BRING_ITEM_TO_LOCATION, false,
     "GARAGE", 0, GRAPPLE_GUN, 0},
    {"ENCOUNTER THE NEST", BRING_ITEM_TO_LOCATION, false, "NEST", 0,
     INCINERATOR, 0},
    {"CHECK THE HYPERSLEEP CHAMBER", BRING_ITEM_TO_LOCATION, false,
     "HYPERSLEEP", 0, MOTION_TRACKER, 0},
    {"GIVE IT A LITTLE INCENTIVE", BRING_ITEM_TO_LOCATION, false, "GALLEY", 0,
     ELECTRIC_PROD, 0},
};

char *final_mission_names[NUM_FINAL_MISSIONS] = {
    "You Have My Sympathies", "Escape On The Narcissus",
    "Blow It Out Into Space", "We're Going To Blow Up The Ship",
    "Cut Off Every Bulkhead And Vent"};

char *final_mission_desc[NUM_FINAL_MISSIONS] = {
    "Defeat Ash, then incinerate the Xenomorph",
    "Assemble all Crew members in DOCKING BAY. DOCKING BAY must have a COOLANT "
    "CANISTER for each Crew member. Crew "
    "must have a CAT CARRIER and INCINERATOR.",
    "Bring the Xenomorph near DOCKING BAY. A Crew member must be in AIRLOCK "
    "and another in BRIDGE, then encounter the "
    "Xenomorph at the end of a turn.",
    "Assemble all Crew members in AIRLOCK with at least 1 Scrap and 1 COOLANT "
    "CANISTER within 4 turns before the "
