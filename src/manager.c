 ...
 * @date   12-May-2022
 * @brief Logic for the game manager, primarily the game loop and some graph
 * functions
 */
...
game_manager *new_game(const arguments args, map *game_map) {
  ...
  if (args.n_characters == 5) {
    for (int i = 0; i < 5; i++) {
      ...
      manager->characters[i]->current_room =
          manager->game_map->player_start_room;
    }
  } else {
    ...
    for (int i = 0; i < args.n_characters; i++) {
      ...
      for (int j = 0; j < 5; j++) {
        ...
        printf("%d) %s, %s - %d Actions - Special Ability: %s\n", j + 1,
               characters[j].last_name, characters[j].first_name,
               characters[j].max_actions, characters[j].ability_description);
      }
      printf("e) Exit\n");
  ...
  for (int i = 0; i < manager->num_objectives; i++) {
    room *tmp =
        get_room(manager->game_map, manager->game_objectives[i].location_name);
    if (tmp != NULL) {
      manager->game_objectives[i].location = tmp;
    } else {
      printf("[WARNING] - Objective room names are hardcoded, should have a "
             "room of name %s.\nSetting location "
             "to %s.\n",
             manager->game_objectives[i].location_name,
             manager->game_map->player_start_room->name);
      manager->game_objectives[i].location =
          manager->game_map->player_start_room;
    }
  }
    ...
    if (!manager->game_objectives[i].completed) {
      ...
      case BRING_ITEM_TO_LOCATION:
        for (int j = 0; j < manager->character_count; j++) {
          if (manager->characters[j]->current_room ==
                  manager->game_objectives[i].location &&
              character_has_item(
                  manager->characters[j],
                  manager->game_objectives[i].target_item_type)) {
      ...
      case CREW_AT_LOCATION_WITH_MINIMUM_SCRAP:;
        ...
        for (int j = 0; j < manager->character_count; j++) {
          if (manager->characters[j]->current_room !=
                  manager->game_objectives[i].location ||
              manager->characters[j]->num_scrap <
                  manager->game_objectives[i].minimum_scrap) {
      ...
      case DROP_COOLANT:;
        ...
        for (int j = 0; j < NUM_ROOM_ITEMS; j++) {
          if (manager->game_objectives[i].location->room_items[j] != NULL &&
              manager->game_objectives[i].location->room_items[j]->type ==
                  COOLANT_CANISTER) {
            coolant_count++;
          }
        }
    ...
    if (all_complete) {
      ...
      do {
        manager->final_mission_type = randint(0, NUM_FINAL_MISSIONS - 1);
          ...
          manager->character_count == 1 &&
          (manager->final_mission_type == CUT_OFF_EVERY_BULKHEAD_AND_VENT ||
...
void setup_final_mission(game_manager *manager) {
  ...
  case YOU_HAVE_MY_SYMPATHIES:;
    ...
    yequipment_storage->num_items =
        max(yequipment_storage->num_items, manager->character_count + 2);
    // Put Ash at MU-TH-UR
  ...
  case ESCAPE_ON_THE_NARCISSUS:;
    ...
    eequipment_storage->num_items =
        max(eequipment_storage->num_items, manager->character_count + 2);
    break;
  ...
  case WERE_GOING_TO_BLOW_UP_THE_SHIP:;
    ...
    wequipment_storage->num_items =
        max(wequipment_storage->num_items, manager->character_count + 2);
  ...
  case CUT_OFF_EVERY_BULKHEAD_AND_VENT:
    ...
    for (int i = 0; i < manager->game_map->named_room_count; i++) {
      manager->game_map->rooms[manager->game_map->named_room_indices[i]]
          ->has_event = true;
    }
    // Give self destruct tracker to active character
    manager->active_character->self_destruct_tracker = 4;
...
void update_final_mission(game_manager *manager) {
  ...
  case YOU_HAVE_MY_SYMPATHIES:
    break;
  case ESCAPE_ON_THE_NARCISSUS:;
    // Win when all members in docking bay with 1 coolant dropped in docking bay
    // each, with cat carrier and incinerator in inventory
    ...
    for (int i = 0; i < NUM_ROOM_ITEMS; i++) {
      if (docking_bay->room_items[i] != NULL &&
          docking_bay->room_items[i]->type == COOLANT_CANISTER) {
        num_canisters++;
      }
    }
    ...
    if (enough_dropped_canisters && has_carrier && has_incinerator &&
        all_in_docking_bay) {
      win_game(manager);
    }
    break;
  case BLOW_IT_OUT_INTO_SPACE:
    // Win if alien is in or adjacent to DOCKING BAY, a crew member is at
    // AIRLOCK and another at BRIDGE, and an alien is encountered (see
    // trigger_encounter)
    break;
  case WERE_GOING_TO_BLOW_UP_THE_SHIP:
    // Win if all members are in airlock with 1 coolant canister and 1 scrap
    // each
    game_won = true;
    ...
    for (int i = 0; i < manager->character_count; i++) {
      if (manager->characters[i]->current_room != airlock ||
          manager->characters[i]->num_scrap == 0 ||
  ...
  case CUT_OFF_EVERY_BULKHEAD_AND_VENT:
    ...
    game_won = true;
    for (int i = 0; i < manager->game_map->named_room_count; i++) {
      if (manager->game_map->rooms[manager->game_map->named_room_indices[i]]
              ->has_event) {
        game_won = false;
      }
    }
 ...
 * @param  allowed_moves               room_queue containing allowed
 * destinations
 * @param  allow_back                  Whether or not to allow the user to exit
 * the movement menu
  ...
  while (1) {
    ...
    if (allowed_moves == NULL) {
      ...
      for (int i = 0; i < to_move->current_room->connection_count; i++) {
        printf("\t%d) %s\n", i + 1,
               to_move->current_room->connections[i]->name);
      }
    ...
    int max_destination_index = allowed_moves == NULL
                                    ? to_move->current_room->connection_count
                                    : allowed_moves->size;
...
/**
 * Find the shortest path between two rooms using a modified Djikstra's
 * algorithm
 ...
 * @param  manager                   Game manager
 * @param  num_spaces                Maximum number of spaces to move the
 * xenomorph
 * @param  morale_drop               Number of morale points to lose if an
 * interception occurs
...
bool xeno_move(game_manager *manager, int num_spaces, int morale_drop) {
  ...
  for (int i = 0; i < manager->character_count; i++) {
    room_queue *rq =
        shortest_path(manager->game_map, manager->xenomorph_location,
                      manager->characters[i]->current_room);
  ...
  if (s < num_spaces) {
    ...
    for (int i = 0; i < num_spaces && shortest->head->room_queue_next != NULL;
         i++) {
      pop_tail(shortest);
    }
...
bool ash_move(game_manager *manager, int num_spaces) {
  ...
  int s = INT_MAX;
  // Scrap check
  for (int i = 0; manager->final_mission_type != YOU_HAVE_MY_SYMPATHIES &&
                  i < manager->game_map->room_count;
       i++) {
    if (manager->game_map->rooms[i]->num_scrap == 0) {
      continue;
    }

    room_queue *rq = shortest_path(manager->game_map, manager->ash_location,
                                   manager->game_map->rooms[i]);
    ...
    if (manager->characters[i]->current_room == manager->ash_location) {
      ...
      if (manager->final_mission_type != YOU_HAVE_MY_SYMPATHIES) {
        ...
        if (character_has_item(manager->characters[i], COOLANT_CANISTER)) {
          ...
          // Move Ash
          room_queue *ash_locations = find_rooms_by_distance(
              manager->game_map, manager->ash_location, 3, false);
...
void check_ash_health(game_manager *manager) {
  if (manager->is_final_mission &&
    ...
    if (manager->ash_health == 0) {
      ...
      printf("[FINAL OBJECTIVE] - You've killed Ash! Use an INCINERATOR on the "
             "Xenomorph to escape!\n");
    } else {
...
int reduce_morale(game_manager *manager, int lost, bool encountered_alien) {
  ...
  for (int i = 0; i < manager->character_count; i++) {
    for (int j = 0; j < 3; j++) {
      if (manager->characters[i]->held_items[j] != NULL &&
        ...
        flashlight_index = j;
        has_flashlight = true;
      } else if (encountered_alien &&
                 manager->characters[i]->held_items[j] != NULL &&
  ...
  if (has_flashlight ^ has_prod) {
    if (has_flashlight) {
      printf("%s has a FLASHLIGHT. Use it to reduce morale lost by 1? (y/n) ",
             char_has_flashlight->last_name);
      ...
      if (ch == 'y') {
        lost = min(0, lost - 1);
        use_item(char_has_flashlight,
                 char_has_flashlight->held_items[flashlight_index]);
      }
    } else {
      printf(
          "%s has an ELECTRIC PROD. Use it to reduce morale lost by 2? (y/n) ",
          char_has_prod->last_name);
    ...
    if (char_has_flashlight == char_has_prod) {
      ...
      printf("%s and %s.", char_has_flashlight->last_name,
             char_has_prod->last_name);
    }
 ...
 * @param  using_motion_tracker  Whether or not the event is being triggered
 * with a motion tracker
 * @return         0 if Safe event, 1 if Jonesy event, 2 if Xenomorph event, -1
 * if no event
 */
int trigger_event(game_manager *manager, struct character *moved,
  ...
  room *target_room =
      is_motion_tracker ? motion_tracker_room : moved->current_room;
    ...
    if (event_type <= 8) {
      ...
      if (manager->jonesy_caught) {
        ...
        for (int i = 0; !is_motion_tracker && i < 3; i++) {
          if (moved->held_items[i] != NULL &&
              moved->held_items[i]->type == CAT_CARRIER) {
            printf("%s has a CAT CARRIER - use it to catch Jonesy? (y/n) ",
                   moved->last_name);
...
void trigger_encounter(game_manager *manager) {
  ...
  if (manager->final_mission_type == BLOW_IT_OUT_INTO_SPACE &&
      encounter >= ALIEN_Lost_The_Signal && encounter <= ALIEN_Hunt) {
    // Win if alien is in or adjacent to DOCKING BAY, a crew member is at
    // AIRLOCK and another at BRIDGE, and an alien is encountered
    room *docking_bay = get_room(manager->game_map, "DOCKING BAY");
    if (docking_bay == NULL) {
      docking_bay = manager->game_map->player_start_room;
    }
    ...
    for (int i = 0; i < docking_bay->connection_count; i++) {
      xeno_in_right_place |=
          manager->xenomorph_location == docking_bay->connections[i];
    }
  ...
  case QUIET:;
    room *target_room =
        manager->game_map->rooms[manager->game_map->named_room_indices[randint(
            0, manager->game_map->named_room_count - 1)]];
    printf("[ENCOUNTER] - All is quiet in %s. Xenomorph moves 1 space.",
           target_room->name);
    if (manager->ash_location != NULL && !manager->ash_killed) {
  ...
  case ORDER937_Meet_Me_In_The_Infirmary:
    if (manager->ash_location != NULL && !manager->ash_killed) {
      printf("[ENCOUNTER] - Meet Me in the Infirmary - Ash moves twice, and %s "
             "moves to %s\n",
  ...
  case ORDER937_Crew_Expendable:
    if (manager->ash_location != NULL && !manager->ash_killed) {
      printf("[ENCOUNTER] - Crew Expendable - Ash moves twice, and %s loses "
             "all Scrap\n",
             manager->active_character->last_name);
    } else {
  ...
  case ORDER937_Collating_Data:
    if (manager->ash_location != NULL && !manager->ash_killed) {
      printf("[ENCOUNTER] - Collating Data - Ash moves twice, and each "
             "character loses 1 "
             "Scrap\n");
    } else {
    ...
    for (int i = 0; i < manager->character_count; i++) {
      manager->characters[i]->num_scrap =
          max(0, manager->characters[i]->num_scrap - 1);
    }
    ash_move(manager, 2);
...
void flee(game_manager *manager, struct character *moved) {
  ...
  room_queue *allowed_moves =
      find_rooms_by_distance(manager->game_map, moved->current_room, 3, false);
        ...
        if (target_item->type == COOLANT_CANISTER) {
          ...
          if (manager->active_character->num_items < 3) {
            ...
            printf("%s picked up the %s\n",
                   manager->active_character->last_name,
                   item_names[manager->active_character->current_room
                                  ->room_items[m]
                                  ->type]);
      ...
      if (scrap_index == selection_index) {
        ...
        if (selection_index == coolant_index) {
          ...
          for (int m = 0; m < NUM_ROOM_ITEMS; m++) {
            if (manager->active_character->current_room->room_items[m] ==
                NULL) {
              manager->active_character->current_room->room_items[m] =
          ...
          for (int l = 0; l < NUM_ROOM_ITEMS; l++) {
            if (manager->active_character->current_room->room_items[l] ==
                NULL) {
 ...
 * Use item handler
 * @param  manager               Game manager
 * @return         0 if break_loop is false, 1 if it's true, 2 if it's true and
 * don't do an encounter this turn
 */
int use(game_manager *manager) {
  ...
  int num_usable = 0;
  for (int i = 0; i < 3; i++) {
    if (manager->active_character->held_items[i] != NULL &&
        manager->active_character->held_items[i]->uses_action) {
  ...
  if (num_usable == 0) {
    ...
    if (ch == 'b') {
      ...
      switch (item_type) {
      case MOTION_TRACKER:;
        room_queue *within_2 = find_rooms_by_distance(
            manager->game_map, manager->active_character->current_room, 2,
            true);

        int event_rooms[64];
        int num_event_rooms = 0;
        for (int i = 0; i < within_2->size; i++) {
          if (poll_position(within_2, i) !=
        ...
        if (num_event_rooms == 0) {
          ...
          for (int i = 0; i < num_event_rooms; i++) {
            printf("\t%d) %s\n", i + 1,
                   poll_position(within_2, event_rooms[i])->name);
          }
          printf("\tb) Back\n");
          ...
          if (ch == 'b') {
            ...
            ch = ch - '0' - 1;
            use_item(manager->active_character,
                     manager->active_character->held_items[item_selection]);
            trigger_event(manager, manager->active_character,
                          poll_position(within_2, event_rooms[ch]));
      ...
      case GRAPPLE_GUN:;
        ...
        if (grapple_q->size > 4) {
          ...
          free(grapple_q);
          room_queue *alien_locations = find_rooms_by_distance(
              manager->game_map, manager->xenomorph_location, 3, false);

          printf("Where to send the Xenomorph to?\n");
          for (int i = 0; i < alien_locations->size; i++) {
            printf("\t%d) %s\n", i + 1,
                   poll_position(alien_locations, i)->name);
          }
          printf("\tb) Back\n");
    ...
    for (int i = 0; i < 5; i++) {
      ...
      if (active->self_destruct_tracker > 0) {
        ...
        if (active->self_destruct_tracker <= 0) {
          ...
          printf("[GAME OVER] - The Nostromo self-destructed with the Crew "
                 "still on it!\n");
          exit(0);
        } else {
        ...
        while (1) {
          ...
          case 'm':; // Start case with assignment
            ...
            if (active->current_room == last_room) {
              ...
              printf("%s moved from %s to %s\n", active->last_name,
                     last_room->name, active->current_room->name);

              // Check for events in new location
              if (trigger_event(manager, active, NULL) ==
                  2) { // Alien encounter
          ...
          case 'a':
            if (!used_ability) {
              ...
              ability_output *ao = active->ability_function(
                  manager->game_map, manager->characters, active);
              ...
              if (ao->move_character_index >= 0) {
                ...
                manager->characters[ao->move_character_index]->current_room =
                    character_move(
                        manager, manager->characters[ao->move_character_index],
                        NULL, false);
          ...
          case 'g':;
            ...
            for (int m = 0; m < manager->character_count; m++) {
              if (manager->characters[m] != active &&
                  manager->characters[m]->current_room ==
                      active->current_room &&
                  ((active->num_items > 0 &&
                    manager->characters[m]->num_items < 3) ||
                   (active->coolant != NULL && active->coolant == NULL) ||
                   active->num_scrap > 0)) {
            ...
            if (num_tradeable == 0) {
              ...
              for (int m = 0; m < num_tradeable; m++) {
                printf("\t%d) %s\n", m + 1,
                       manager->characters[tradeable_indices[m]]->last_name);
              }
              printf("\tb) Back\n");
              ...
              character *give_target =
                  manager->characters[tradeable_indices[ch - '0' - 1]];
              ...
              if (ch != 'b') {
                ...
                if (ch == 'b') {
                  ...
                  if (give_target->coolant == NULL &&
                      ch == 'c') { // Give coolant
                    printf("%s gave COOLANT CANISTER to %s\n",
                           active->last_name, give_target->last_name);
                    ...
                    printf("%s gave %s %d Scrap.\n", active->last_name,
                           give_target->last_name, ch);
                    break_loop = true;
                  } else { // Give item
                    for (int m = 0; m < 3; m++) {
                      if (give_target->held_items[m] == NULL) {
                        give_target->held_items[m] =
                            active->held_items[ch - '0' - 1];
          ...
          case 'n':
            if (manager->final_mission_type == BLOW_IT_OUT_INTO_SPACE) {
              if (manager->active_character->num_scrap == 0) {
                ...
                ability_output *ao = lambert_ability(
                    manager->game_map, manager->characters, active);
          ...
          case 'r':
            ...
            break;
          case 'e':
            printf("Are you sure you want to exit? Game progress will not be "
                   "saved. "
                   "(y/n)\n");
            if (get_character() == 'y') {
              exit(0);
